...
// TestActivateDropsClientAuthorization reproduces the Solar Winds 401: every
...
// header as authoritative rejects it. The bridge reaches an Aperture whose
...
func TestActivateDropsClientAuthorization(t *testing.T) {
	backend := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	resp.Body.Close()
	if resp.StatusCode != http.StatusNoContent {
		t.Fatalf("status = %d, want %d: Authorization crossed the bridge proxy", resp.StatusCode, http.StatusNoContent)
	}
}
